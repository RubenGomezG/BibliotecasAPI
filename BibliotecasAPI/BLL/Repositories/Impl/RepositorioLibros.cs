...
namespace BibliotecasAPI.BLL.Repositories.Impl
{
    public class RepositorioLibros : IRepositorioLibros
    {
        ...
        {
            IQueryable<Libro> queryable = _context.Libros
                            .Include(l => l.Autores)
                            .ThenInclude(a => a.Autor)
                            .AsQueryable();

            await _httpContextAccessor.HttpContext!.InsertarParametrosPaginacionEnCabecera(queryable);
            List<Libro> libros = await queryable
                            .OrderBy(a => a.Titulo)
                            .Paginar(paginacionDTO)
                            .ToListAsync();

            IEnumerable<LibroConAutoresDTO> librosDTO = _mapper.Map<IEnumerable<LibroConAutoresDTO>>(libros);
            return librosDTO;
        }
        ...
        public async Task<ActionResult<LibroConAutoresDTO>> GetLibroPorId(int id)
        {
            ...
            LibroConAutoresDTO libroDTO = _mapper.Map<LibroConAutoresDTO>(libro);
            return libroDTO;
        }
        ...
        {
            int registrosBorrados = await _context.Libros.Where(a => a.Id == id).ExecuteDeleteAsync();
